...
def main(argv):
  op = optparse.OptionParser(description = U"ѕроверка на соответствие XSD", prog="xsd", version="0.1", usage=U"%prog")
  op.add_option("-x", "--xml", dest="xml", help=U"XML документ", metavar="XML_FILE")
  op.add_option("-s", "--xsd", dest="dtd", help=U"XSD документ", metavar="XSD_FILE")
